...
export class TestExecutor {
  ...
  async runTestFromJSON(file: string) {
      ...
      while (!success && retries < 3) {
        try {
          ...
          if (retries >= 3) {
            ...
            await this.captureDebugArtifacts(page, i + 1);
            throw e;
          }
  ...
  /* ---------- DEBUG HELPERS ---------- */
  private async captureDebugArtifacts(page: Page, stepIndex: number) {
